...
В этот раз я постараюсь все ужать до минимума и обходиться без видео в тексте. Но про видео будет отдельное объявление в разделе "Видео".
...
![vostok_prediction](images/vostok6_prediction.png)

А реальность была такова:

![rescue0](images/spot_trek.png)

В этот раз разница между ожидаемой точкой посадки и реальной - серьезная. Можно сказать, что совпало только направление. В итоге: отлетел на 50км относительно точки старта. Местность - какой-то лесок. На спутниковых картах "вроде как" лежит на опушке. Легкая прогулочка, тем более что у нас есть вот этот парень:
...
С GPS попрощался на высоте примерно 500м. Вернулся он уже только на земле. Допуская, что подъем был равномерным, я продолжил последовательность данных GPS в Excel до времени, соответствующему времени разрыва шара на видео - это точка максимальной высоты. Я также добавил погрешность в 10%, чтобы видеть поле возможных значений. Таким образом мы получили высоту 20-25 км. Берем посередине, получаем **22км** - именно это значение используем для видео и отчетности.
...
GPS потух на 1.5 км. Считаем как в первом случае и видим, что... Тут получше - 28-36 км. Берем **31км** - это уже похоже на ожидаемый результат (хотя у нас бывало и повыше).
