...
Тем не менее, координаты приземления мы получили для Vsotok-6b раньше (Spot Trace) и отправились смотреть его.
...
[vostok6b_found](https://www.youtube.com/watch?v=M8rKQkKHYKg&ab_channel=Vostok)
...
Да, нет записи возвращения на землю (а что есть, поговорим в разделе про видео). Итак, моя версия: мы в этот раз записывали в разрешении 5.7k. Карта памяти осталась та же и она прекрасно справлялась с записью 4k. Мы проверяли на земле - она и с 5.7k прекрасно справлялась пока... камерой не начинаешь активно трясти (это мы выяснили уже после поисков) - тогда появляется надпись "Медленная карта памяти. Запись остановлена" (или как-то так, точно не помню). Такой подставы никто не ожидал (опять) и никому и в голову не могло придти, что скорость записи видео как-то зависит от того что снимаешь. Мне кажется, что чем активнее действие в кадре - тем больше скорость записи нужна и это обусловлено особенностями кодирования видео. В режиме 4к карта справлялась с любой динамикой происходящего, в режиме 5.7k - только пока был спокойный подъем. А как начался бешаный спуск - карта потерялась.  
...
С GPS попращался на высоте примерно 500м. Вернулся он уже только на земле. Допуская, что подъем был равномерным, я продолжил последовательность данных GPS в Excel до времени, соответствующему времени разрыва шара на видео - это точка максимальной высоты. Я так же добавил погрешность в 10%, чтобы видеть поле возможных значений. Таким образом мы получили высоту 20-25км. Берем посередине, получаем **23км** - именно это значение используем для видео и отчетности.
